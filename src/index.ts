 ...
 */
export function systemic<TMaster extends Record<string, unknown> = EmptyObject>(options?: { name?: string }): Systemic<TMaster> {
  const api = {};
  const params = Object.assign({}, { name: randomName() }, options);
  let definitions = {};
  ...
  function add(...args) {
    ...
    if (definitions.hasOwnProperty(name)) throw new Error(`Duplicate component: ${name}`);
