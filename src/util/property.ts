import type { PropAt } from "../types/util";

function hasDirectProperty<T>(source: T, key: PropertyKey): key is keyof T {
  if (!source) return false;
  return key in (source as any);
}

export function hasProperty(source: any, key: string): boolean {
  if (hasDirectProperty(source, key) as boolean) {
    return true;
  }
...
export function getProperty<T, K extends string>(source: T, key: K): PropAt<T, K> {
  if (hasDirectProperty(source, key)) return source[key] as PropAt<T, K>;
  if (source && key.includes(".")) {
    const keyParts = key.split(".");
