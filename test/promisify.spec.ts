...
describe('promisify', () => {
  describe('asCallbackSystem', () => {
    it('converts a systemic system to a callback system', done => {
      const system = {
    ...
    it('calls the callback with an error when starting the system fails', done => {
      const system = {
    ...
    it('calls the callback with an error when stopping the system fails', done => {
      const system = {
        start: jest.fn().mockResolvedValue('started'),
    ...
    it('calls the callback with an error when restarting the system fails', done => {
      const system = {
        start: jest.fn().mockResolvedValue('started'),
        stop: jest.fn().mockResolvedValue('stopped'),
  ...
  describe('promisifyComponent', () => {
    it('promisifies a callback component', async () => {
      const callbackComponent = {
        start: (dependencies: { foo: string }, callback: (err: any, component: { foo: string; bar: string }) => void) => {
    ...
    it('returns a rejected promise when starting the promisified component fails', async () => {
      const callbackComponent = {
        start: (dependencies: { foo: string }, callback: (err: any, component: { foo: string; bar: string } | null) => void) => {
